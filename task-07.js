...
//проверь количество символов параметра login и верни true или false в зависимости от того,
//попадает ли длина параметра в заданный диапазон от 4-х до 16-ти символов включительно.
const isLoginValid = function(login) {
  // твой код
  let len = login.length;
  return ( len >3 && len < 17 ) ? true : false;
};

// принимает список всех логинов и добавляемый логин как параметры и проверяет наличие login в массиве allLogins,
// возвращая true если такого логина еще нет и false если логин уже используется.
const isLoginUnique = function(allLogins, login) {
  // твой код
  for (const elem of allLogins) {
    if (elem === login) return false;
  }
  return true;
};
